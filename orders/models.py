...
from socialiga.utils import unique_order_id_generator
...
class OrderManagerQuerySet(models.query.QuerySet):
    ...
    def by_weeks_range(self, weeks_ago=7, number_of_weeks=2):
        number_of_weeks = min(number_of_weeks, weeks_ago)
...
class Order(models.Model):
    ...
    def check_done(self):
        ...
        shipping_done = False
        if self.shipping_address or not shipping_address_required:
            shipping_done = True
        else:
            shipping_done = False
        ...
        total = self.total
        return bool(
            billing_profile and shipping_done and billing_address and total > 0
        )
    ...
    def mark_paid(self):
        if self.status != 'paid' and self.check_done():
            self.status = "paid"
            self.save()
...
class ProductPurchaseManager(models.Manager):
    ...
    def products_by_id(self, request):
        ...
        return [x.product.id for x in qs]

    def products_by_request(self, request):
        ids_ = self.products_by_id(request)
        return Product.objects.filter(id__in=ids_).distinct()
